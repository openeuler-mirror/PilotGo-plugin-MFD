...
class ExtFrag:
    ...
    def calculate_scoreA(self, score_a):
        ...
        formatted_score_a = f"{score_a_int_part:2d}.{score_a_dec_part:03d}"
        return formatted_score_a

    def calculate_scoreB(self, score_b):
        score_b_int_part = int(score_b) // 1000
        score_b_dec_part = int(score_b) % 1000
        formatted_score_b = f"{score_b_int_part:2d}.{score_b_dec_part:03d}"
        return formatted_score_b
    ...
    def get_node_data(self):
        ...
        for key, value in pgdat_map.items():
            ...
            node_id = value.node_id
            node_data_dict[node_id] = data  # 将数据存储到字典中，以 node_id 作为键

        return node_data_dict
       
 # 定义输出回调函数
    def print_event11(self,cpu, data, size):
    ...
    def get_event_data(self):
        ...
        return events_data  # 确保函数始终返回 events_data 列表
