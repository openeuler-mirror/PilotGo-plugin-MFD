...
def create_zone_table2(extfrag, args):
    rows = []
    base_header = ["COMM", "ZONE_PFN", "SUM_PAGES", "FACT_PAGES", 
                  "ORDER", "TOTAL", "SUITABLE", "FREE", "NODE_ID", "SCORE1", "SCORE2"]
    score_columns = []
    ...
    for comm, zones in zone_data.items():
        for zone in zones:
            ...
            for col in columns:
                value = str(zone.get(col, ""))
                if col == "order" and int(value) >= 5 and float(zone.get("scoreB", 0)) >= 0.5:
                    text = urwid.Text(value, align='center')
                    clickable_text = urwid.AttrMap(ClickableText(text, col, value), 'critical', focus_map='highlight')
                else:
                    text = urwid.Text(value, align='center')
            ...
            if args.bar:
                ...
                frag_bar = generate_fragmentation_bar(score)
                frag_bar_widget = ClickableText(urwid.Text(frag_bar, align='center'), "FRAG_BAR", frag_bar)
                cell_widgets.append(urwid.AttrMap(frag_bar_widget, 'body', focus_map='highlight'))
...
def create_zone_table1(extfrag, args):
    """构建并显示Zone信息"""
    rows = []
    base_header = ["COMM", "ZONE_PFN", "SUM_PAGES", "FACT_PAGES", 
